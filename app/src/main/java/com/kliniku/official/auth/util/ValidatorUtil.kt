...
object ValidatorUtil {
    ...
    fun validateEmail(context: Context, email: String): ValidationResult {
        return when {
            ...
            !Patterns.EMAIL_ADDRESS.matcher(email).matches() -> ValidationResult(
                false,
                context.getString(R.string.error_email_invalid_format)
            )
    ...
    fun validatePhone(context: Context, phone: String): ValidationResult {
        return when {
            ...
            !phone.matches(Regex("^[0-9]{10,13}$")) -> ValidationResult(
                false,
                context.getString(R.string.error_phone_length)
            )

            !phone.startsWith("08") -> ValidationResult(
                false,
                context.getString(R.string.error_phone_prefix)
            )
    ...
    fun validateBirthdate(context: Context, birthdate: String?, date: Date?): ValidationResult {
        ...
        return when {
            age < 17 -> ValidationResult(
                false,
                context.getString(R.string.error_birthdate_too_young)
            )

            age > 100 -> ValidationResult(
                false,
                context.getString(R.string.error_birthdate_invalid)
            )
    ...
    fun validatePassword(context: Context, password: String): ValidationResult {
        return when {
            ...
            !password.matches(Regex(".*[A-Z].*")) -> ValidationResult(
                false,
                context.getString(R.string.error_password_uppercase)
            )

            !password.matches(Regex(".*[a-z].*")) -> ValidationResult(
                false,
                context.getString(R.string.error_password_lowercase)
            )

            !password.matches(Regex(".*[0-9].*")) -> ValidationResult(
                false,
                context.getString(R.string.error_password_number)
            )
